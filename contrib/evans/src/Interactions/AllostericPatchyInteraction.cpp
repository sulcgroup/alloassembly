...
void AllostericPatchyInteraction::step() {
...
number AllostericPatchyInteraction::_spherical_patchy_two_body(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
...
number AllostericPatchyInteraction::_patchy_two_body_point(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
    number sqr_r = _computed_r.norm();
    if(sqr_r > _sqr_rcut) {
        return (number) 0.f;
    }
        ...
        for(uint q_patch_idx = 0; q_patch_idx < q->N_int_centers(); q_patch_idx++) {
            ...
            if ((p_patch->bound || p_patch->is_active()) && (q_patch->bound || q_patch->is_active())) {
                ...
                if (dist < _sqr_patch_rcut) {
                    ...
                    number epsilon = p_patch->color() + q_patch->color() == 0 ? 1 : 0;
...
number AllostericPatchyInteraction::_patchy_two_body_KF(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
    number sqr_r = _computed_r.norm();
    if(sqr_r > _sqr_rcut) {
        return (number) 0.f;
    }
...
number AllostericPatchyInteraction::_three_body(BaseParticle *p, PatchyBond &new_bond, bool update_forces) {
    number energy = 0.;
...
void AllostericPatchyInteraction::begin_energy_computation() {
...
number AllostericPatchyInteraction::pair_interaction(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
    if(compute_r) {
...
number AllostericPatchyInteraction::pair_interaction_bonded(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
    number energy = 0.;

    return energy;
}

number AllostericPatchyInteraction::pair_interaction_nonbonded(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
    if(compute_r) {
        _computed_r = _box->min_image(p->pos, q->pos);
    }
...
void AllostericPatchyInteraction::allocate_particles(std::vector<BaseParticle*> &particles) {
...
//void AllostericPatchyInteraction::_parse_interaction_matrix() {
...
void AllostericPatchyInteraction::_load_patchy_particle_files(std::string &patchy_file, std::string &particle_file)
{
...
//    getInputString(obs_input, "allostery_conditional", allostery_conditional, 1); //1?
    ...
    AllostericPatch loaded_patch(a1, position, id, color, state_var, activation_var);
 ...
 * @param ptype
 */
void AllostericPatchyInteraction::_process_state_transition_map(int ptype) {
...
void AllostericPatchyInteraction::read_topology(int *N_strands, std::vector<BaseParticle*> &particles) {
    _N = particles.size();
    *N_strands = _N;
    int N_types;
...
void AllostericPatchyInteraction::check_input_sanity(std::vector<BaseParticle*> &particles) {

}

extern "C" AllostericPatchyInteraction* make_AllostericPatchyInteraction() {
    return new AllostericPatchyInteraction();
}
