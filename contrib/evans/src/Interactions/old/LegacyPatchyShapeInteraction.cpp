...
bool PatchyShapeInteraction<number>::_patches_compatible(PatchyShapeParticle<number>  *p, PatchyShapeParticle<number>  *q, int pi, int pj ) {
    if(abs(p->patches[pi]._color) < 10 && abs(q->patches[pj]._color ) < 10  )  //we are in the self-complementary regime
    {
        if(p->patches[pi]._color == q->patches[pj]._color ) { //patches are the same, hence complementary
            return true;
        }
        else {
            return false;
    ...
    else if (p->patches[pi]._color + q->patches[pj]._color == 0) { //the case where only complementary patches (like -10 and +10) can bind
        return true;
            ...
            for(int j = i; j < _N_particle_types; j++)
            {
                ...
                for(int pi = 0; pi < p->N_patches; pi++)
                {
                    for(int qj = 0; qj < q->N_patches; qj++)
                    {
                        ...
                        if(p->patches[pi]._color == q->patches[qj]._color)
                        {
