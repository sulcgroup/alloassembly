/*
 * PDClusterTopology.cpp
 *
 *  Created on: May 5, 2022
 *      Author: josh evans
 *  Initially a copy of PLClusterTopology.cpp, with modifications to output more complex cluster data
 */

#include "PDClusterTopology.h"
...
//Edge struct and Graph class copied directly from PLPatchyTopology
...
PDClusterTopology<number>::PDClusterTopology() {
 _show_types = true;
}
...
template<typename number>
void PDClusterTopology<number>::get_settings(input_file &my_inp, input_file &sim_inp) {
	int show_types = 0;
...
template<typename number>
std::string PDClusterTopology<number>::get_output_string(llint curr_step) {
...
template class PDClusterTopology<float>;
template class PDClusterTopology<double>;
